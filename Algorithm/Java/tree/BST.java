...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    public Key floor(Key key) {
        ...
        if (isEmpty()) {
            throw new NoSuchElementException("calls floor() with empty symbol");
        }
    ...
    public Key ceiling(Key key) {
        ...
        if (isEmpty())
            throw new NoSuchElementException("calls celling() with empty symbol");
    ...
    private void keys(Node x, Queue<Key> queue, Key lo, Key hi) {
        ...
        if (cmplo <= 0 && cmphi >= 0) queue.enqueue(x.key); // enqueue: 入队
